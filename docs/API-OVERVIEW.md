...
- `POST /api/remote/devices/{deviceId}/commands` — send a remote command through the adaptive routing layer
...
- `GET /api/remote/discovery/candidates/{candidateId}/pairing-suggestions` — show recommended control-path suggestions for a candidate device
...
- `GET /api/remote/devices/{deviceId}/onboarding/status` — summarize the latest onboarding state for a device
